...
namespace Kanboard\Plugin\Wechat\Notification;

require_once dirname(__DIR__).'/SDK/Wechat.php';
...
class Telegram extends Base implements NotificationInterface
{
    ...
    protected function sendMessage($apikey, $bot_username, $forward_attachments, $chat_id, array $project, $eventName, array $eventData)
    {
        ...
        //发送微信消息
         ...
         $this->sendCustomMessage($chat_id,$message);
        return true;
    ...
    protected function sendWechatMessage($apikey, $bot_username, $forward_attachments, $chat_id, array $project, $eventName, array $eventData)
    {
        if ($this->userSession->isLogged())
        {
            $author = $this->helper->user->getFullname();
        ...
        try{
            ...
            file_put_contents('wechat_log.log',$exception->getMessage()."\n\t",FILE_APPEND);
        }
    }

    public function sendCustomMessage($openId,$message,$type = 'text')
    {
        return wechat()->sendCustomMessage([
            'touser'=>$openId,
            'msgtype'=>"text",
