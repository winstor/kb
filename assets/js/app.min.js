(function() {
    ...
    function getCaretCoordinates(element, position, options) {
        ...
        var debug = options && options.debug || !1;
        if (debug) {
...
KB.onClick = function(selector, callback, noPreventDefault) {
    this.listeners.clicks[selector] = {
        callback: callback,
        noPreventDefault: noPreventDefault === !0
    }
};
...
KB.onKey = function(combination, callback, ignoreInputField, ctrlKey, metaKey) {
    this.listeners.keys.push({
        ...
        'callback': callback,
        'ignoreInputField': ignoreInputField || !1,
        'ctrlKey': ctrlKey || !1,
        'metaKey': metaKey || !1
    })
};
KB.listen = function() {
    ...
    function onKeyPressed(e) {
        ...
        if (keysQueue.length > 0) {
            ...
            for (var i = 0; i < self.listeners.keys.length; i++) {
                ...
                if (KB.utils.arraysIdentical(keysQueue, sequence) && e.ctrlKey === params.ctrlKey && e.metaKey === params.metaKey) {
    ...
    document.addEventListener('click', onClick, !1);
    document.addEventListener('change', onChange, !1);
    window.addEventListener('keydown', onKeyPressed, !1)
};
...
KB.html.radio = function(label, name, value) {
    return KB.dom('label').add(KB.dom('input').attr('type', 'radio').attr('name', name).attr('value', value).build()).text(label).build()
};
...
KB.http.postForm = function(url, formElement) {
    ...
    return (new KB.http.request('POST', url, {},
    formData)).execute()
};
KB.http.uploadFile = function(url, file, csrf, onProgress, onComplete, onError, onServerError) {
    ...
    function afterRendering() {
        ...
        if (formElement) {
            formElement.on('submit', onFormSubmit, !1)
        }
    ...
    function replace(html) {
        ...
        if (contentElement) {
            contentElement.replace(KB.dom('div').attr('id', 'modal-content').html(html).build());
            afterRendering()
        }
    }
    function create(html, width, overlayClickDestroy) {
        var closeButtonElement = KB.dom('a').attr('href', '#').attr('id', 'modal-close-button').html('<i class="fa fa-times"></i>').click(onCloseButtonClick).build();
        var headerElement = KB.dom('div').attr('id', 'modal-header').add(closeButtonElement).build();
        var contentElement = KB.dom('div').attr('id', 'modal-content').html(html).build();
        var boxElement = KB.dom('div').attr('id', 'modal-box').style('width', width).add(headerElement).add(contentElement).build();
        var blurPicElement = KB.dom('div').attr('id', 'modal-blurpic').add(boxElement).build();
        var overlayElement = KB.dom('div').attr('id', 'modal-overlay').add(boxElement).build();
        if (overlayClickDestroy) {
            blurPicElement.addEventListener('click', onOverlayClick, !1)
            overlayElement.addEventListener('click', onOverlayClick, !1)
        }
        document.body.appendChild(blurPicElement);
        document.body.appendChild(overlayElement);
        afterRendering()
    }
    function destroy() {
        isOpen = !1;
        var blurPicElement = KB.find('#modal-blurpic');
        if (blurPicElement) {
            KB.trigger('modal.beforeDestroy');
            blurPicElement.remove()
        }
...
KB.tooltip = function() {
    ...
    function mouseOnTooltip() {
        document.getElementById("tooltip-container").mouseOnTooltip = !0
    }
    ...
    function render(element, html) {
        ...
        containerElement.addEventListener("mouseleave", onMouseLeaveContainer, !1);
        containerElement.addEventListener("mouseenter", mouseOnTooltip, !1);
        containerElement.mouseOnTooltip = !1;
        var elementRect = element.getBoundingClientRect();
        var top = elementRect.top + window.scrollY + elementRect.height+10;
    ...
    for (var i = 0; i < elements.length; i++) {
        elements[i].addEventListener("mouseenter", onMouseOver, !1);
        elements[i].addEventListener("mouseleave", mouseLeftParent, !1)
    }
};
...
KB.onClick('.js-autocomplete-email',
function(e) {
...
KB.onClick('.js-autocomplete-subject',
function(e) {
    ...
    KB.onClick('.task-board *', redirectToTaskView, !0);
    KB.onClick('.task-board-change-assignee *', openEditTask, !0)
} ());
KB.component('chart-project-avg-time-column',
...
KB.component('chart-project-burndown',
function(containerElement, options) {
    this.render = function() {
        var metrics = options.metrics;
...
KB.component('chart-project-cumulative-flow',
function(containerElement, options) {
    this.render = function() {
        var metrics = options.metrics;
        var columns = [];
...
KB.component('chart-project-lead-cycle-time',
function(containerElement, options) {
    this.render = function() {
        var metrics = options.metrics;
...
KB.component('chart-project-task-distribution',
function(containerElement, options) {
    this.render = function() {
        var columns = [];
...
KB.component('chart-project-time-comparison',
function(containerElement, options) {
    this.render = function() {
...
KB.component('chart-project-user-distribution',
function(containerElement, options) {
    this.render = function() {
        var columns = [];
        for (var i = 0; i < options.metrics.length; i++) {
...
KB.component('chart-task-time-column',
function(containerElement, options) {
    this.render = function() {
        var metrics = options.metrics;
        var plots = [options.label];
...
function(containerElement, options) {
    ...
    function buildButton() {
        var button = KB.dom('button').click(onSubmit).attr('id', 'modal-confirm-button').attr('type', 'button').attr('class', 'btn btn-red');
        if (isLoading) {
            button.disable().add(KB.dom('i').attr('class', 'fa fa-spinner fa-pulse').build()).text(' ')
        }
        if (options.tabindex) {
    ...
    this.render = function() {
        ...
        var element = KB.dom('div').attr('class', 'form-actions').add(buildButton()).text(' ' + options.orLabel + ' ').add(KB.dom('a').attr('href', '#').click(onCancel).text(options.cancelLabel).build()).build();
        containerElement.appendChild(element)
    }
});
KB.component('external-task-view',
function(containerElement, options) {
    this.render = function() {
...
function(containerElement, options) {
    ...
    function onComplete() {
        ...
        if (currentFileIndex < files.length) {
            ...
            var alertElement = KB.dom('div').addClass('alert').addClass('alert-success').text(options.labelSuccess).build();
            var buttonElement = KB.dom('button').attr('type', 'button').addClass('btn').addClass('btn-blue').click(onCloseWindow).text(options.labelCloseSuccess).build();
    ...
    function showFiles() {
        if (files.length > 0) {
            ...
            KB.dom(dropzoneElement).empty().add(buildFileListElement())
        } else {
            KB.trigger('modal.disable');
            KB.dom(dropzoneElement).empty().add(buildInnerDropzoneElement())
        }
    }
    function buildFileInputElement() {
        return KB.dom('input').attr('id', 'file-input-element').attr('type', 'file').attr('name', 'files[]').attr('multiple', !0).on('change', onFileChange).hide().build()
    }
    function buildInnerDropzoneElement() {
        var dropzoneLinkElement = KB.dom('a').attr('href', '#').text(options.labelChooseFiles).click(onClickFileBrowser).build();
    ...
    function buildDropzoneElement() {
        var dropzoneElement = KB.dom('div').attr('id', 'file-dropzone').add(buildInnerDropzoneElement()).build();
    ...
    function buildFileListItem(index) {
        var isOversize = !1;
        var progressElement = KB.dom('progress').attr('id', 'file-progress-' + index).attr('value', 0).build();
        var percentageElement = KB.dom('span').attr('id', 'file-percentage-' + index).text('(0%)').build();
        var deleteElement = KB.dom('span').attr('id', 'file-delete-' + index).html('<a href="#"><i class="fa fa-trash fa-fw"></i></a>').on('click',
        function() {
            files.splice(index, 1);
            KB.find('#file-item-' + index).remove();
            showFiles()
        }).build();
        var itemElement = KB.dom('li').attr('id', 'file-item-' + index).add(deleteElement).add(progressElement).text(' ' + files[index].name + ' ').add(percentageElement);
    ...
    function buildFileListElement() {
        var fileListElement = KB.dom('ul').attr('id', 'file-list').build();
...
function(containerElement, options) {
    ...
    function renderSlide() {
        var closeElement = KB.dom('div').attr('class', 'fa fa-window-close slideshow-icon slideshow-close-icon').build();
        var downloadElement = KB.dom('a').attr('class', 'fa fa-download slideshow-icon slideshow-download-icon').attr('href', getUrl(currentImage, 'download')).build();
        var previousElement = KB.dom('div').attr('class', 'fa fa-chevron-circle-left slideshow-icon slideshow-previous-icon').build();
        var nextElement = KB.dom('div').attr('class', 'fa fa-chevron-circle-right slideshow-icon slideshow-next-icon').build();
        var imageElement = KB.dom('img').attr('src', getUrl(currentImage, 'image')).attr('alt', currentImage.name).attr('title', currentImage.name).style('maxHeight', (window.innerHeight - 50) + 'px').build();
        var captionElement = KB.dom('figcaption').text(currentImage.name).build();
        var figureElement = KB.dom('figure').add(imageElement).add(captionElement).build();
        var overlayElement = KB.dom('div').addClass('image-slideshow-overlay').add(closeElement).add(downloadElement).add(previousElement).add(nextElement).add(figureElement).click(onOverlayClick).build();
        document.body.appendChild(overlayElement);
    ...
    function buildThumbnailElement(image) {
        return KB.dom('img').attr('src', getUrl(image, 'thumbnail')).attr('alt', image.name).attr('title', image.name).data('imageId', image.id).click(onClick).build()
    }
    this.render = function() {
...
KB.keyboardShortcuts = function() {
    ...
    KB.onKey('Enter', submitForm, !0, !0);
    KB.onKey('Enter', submitForm, !0, !1, !0);
    KB.onKey('b',
    function() {
        if (!KB.modal.isOpen()) {
    ...
    if (KB.exists('#board')) {
        ...
        function() {
            if (!KB.modal.isOpen()) {
                KB.modal.open(KB.find('#board').data('taskCreationUrl'), 'large', !1)
            }
        })
    }
    if (KB.exists('#task-view')) {
        ...
        function() {
            if (!KB.modal.isOpen()) {
                KB.modal.open(KB.find('#task-view').data('editUrl'), 'large', !1)
            }
        });
        KB.onKey('c',
        function() {
            if (!KB.modal.isOpen()) {
                KB.modal.open(KB.find('#task-view').data('commentUrl'), 'medium', !1)
            }
        });
        KB.onKey('s',
        function() {
            if (!KB.modal.isOpen()) {
                KB.modal.open(KB.find('#task-view').data('subtaskUrl'), 'medium', !1)
            }
        });
        KB.onKey('l',
        function() {
            if (!KB.modal.isOpen()) {
                KB.modal.open(KB.find('#task-view').data('internalLinkUrl'), 'medium', !1)
            }
        })
    }
    KB.onKey('f',
...
function() {
    ...
    KB.on('dropdown.afterRender',
    function() {
        var actionLinks = document.querySelectorAll("a[data-list-item-action=modal]");
        for (var i = 0; i < actionLinks.length; i++) {
            actionLinks[i].addEventListener("click", onActionClick, !1)
        }
    })
}); (function() {
    function getLink(e) {
    ...
    KB.onClick('.js-modal-large',
    function(e) {
        KB.modal.open(getLink(e), 'large', !1)
    });
    KB.onClick('.js-modal-medium',
    function(e) {
        if (KB.modal.isOpen()) {
            KB.modal.replace(getLink(e))
        } else {
            KB.modal.open(getLink(e), 'medium', !1)
        }
    });
    KB.onClick('.js-modal-small',
    function(e) {
        KB.modal.open(getLink(e), 'small', !1)
    });
    KB.onClick('.js-modal-confirm',
    function(e) {
        KB.modal.open(getLink(e), 'small')
    });
    KB.onClick('.js-modal-close',
    function() {
        KB.modal.close()
    });
    KB.onClick('.js-modal-replace',
    function(e) {
        var link = getLink(e);
        if (KB.modal.isOpen()) {
...
KB.onChange('.js-project-creation-select-options',
function(element) {
...
KB.component('project-select-role',
function(containerElement, options) {
    var isLoading = !1;
    var isSuccess = !1;
    var isError = !1;
    ...
    function buildComponentElement() {
        ...
        container.add(KB.dom('select').change(onChange).
        for ('option', roles).build());
        if (isLoading) {
            container.text(' ');
...
function(containerElement) {
    ...
    function initialize() {
        ...
        if (!window.Clipboard) {
            ...
            pasteCatcher.contentEditable = !0;
    ...
    this.render = function() {
        inputElement = KB.dom('input').attr('type', 'hidden').attr('name', 'screenshot').build();
        containerElement.appendChild(inputElement);
        initialize()
    }
});
KB.component('select-dropdown-autocomplete',
function(containerElement, options) {
    ...
    function buildDropdownMenu() {
        ...
        return KB.dom('ul').attr('id', 'select-dropdown-menu').style('top', (windowPosition + componentPosition.bottom) + 'px').style('left', componentPosition.left + 'px').style('width', componentPosition.width + 'px').style('maxHeight', (window.innerHeight - componentPosition.bottom - 20) + 'px').mouseover(onItemMouseOver).click(onItemClick).
        for ('li', itemElements).build()
    }
    ...
    this.render = function() {
        ...
        chevronIconElement = KB.dom('i').attr('class', 'fa fa-chevron-down select-dropdown-chevron').click(toggleDropdownMenu).build();
        loadingIconElement = KB.dom('span').hide().addClass('select-loading-icon').add(KB.dom('i').attr('class', 'fa fa-spinner fa-pulse').build()).build();
        inputHiddenElement = KB.dom('input').attr('type', 'hidden').attr('name', options.name).attr('value', options.defaultValue || '').build();
        inputElement = KB.dom('input').attr('type', 'text').attr('placeholder', getPlaceholderValue()).addClass('select-dropdown-input').on('focus', toggleDropdownMenu).on('input', onInputChanged, !0).build();
        componentElement = KB.dom('div').addClass('select-dropdown-input-container').add(inputHiddenElement).add(inputElement).add(chevronIconElement).add(loadingIconElement).build();
        containerElement.appendChild(componentElement);
        if (options.onFocus) {
...
KB.component('submit-buttons',
function(containerElement, options) {
    var isLoading = !1;
    var isDisabled = options.disabled || !1;
    ...
    function buildButton() {
        var button = KB.dom('button').attr('type', 'submit').attr('class', 'btn btn-' + (options.color || 'blue'));
        if (KB.modal.isOpen()) {
    ...
    this.render = function() {
        ...
        var formActionElementBuilder = KB.dom('div').attr('class', 'form-actions').add(submitButtonElement);
        if (KB.modal.isOpen()) {
            formActionElementBuilder.text(' ' + options.orLabel + ' ').add(KB.dom('a').attr('href', '#').click(onCancel).text(options.cancelLabel).build())
        }
...
KB.on('dom.ready',
function() {
    $(document).on('click', '.js-subtask-toggle-status',
    function(e) {
        var el = $(this);
    ...
    $(document).on('click', '.js-subtask-toggle-timer',
    function(e) {
        var el = $(this);
        e.preventDefault();
        $.ajax({
...
function(containerElement, options) {
    ...
    function renderMenu(items) {
        ...
        document.addEventListener('keydown', onKeyDown, !1);
        var menu = KB.dom('ul').attr('id', 'suggest-menu').click(onClick).mouseover(onMouseOver).style('left', left + 'px').style('top', top + 'px').
        for ('li', items).build();
...
KB.component('task-move-position',
function(containerElement, options) {
    ...
    function onError(message) {
        KB.trigger('modal.stop');
        KB.find('#message-container').replace(KB.dom('div').attr('id', 'message-container').attr('class', 'alert alert-error').text(message).build())
    }
    function onSubmit() {
    ...
    function buildSwimlaneSelect() {
        ...
        return KB.dom('select').attr('id', 'form-swimlanes').change(onSwimlaneChanged).
        for ('option', swimlanes).build()
    }
    function buildColumnSelect() {
        ...
        return KB.dom('select').attr('id', 'form-columns').change(onColumnChanged).
        for ('option', columns).build()
    }
    function buildTasks() {
        var tasks = [];
        var swimlaneId = getSwimlaneId();
        ...
        if (tasks.length > 0) {
            container.add(KB.html.label(options.positionLabel, 'form-position')).add(KB.dom('select').attr('id', 'form-position').
            for ('option', tasks).build()).add(KB.html.radio(options.beforeLabel, 'positionChoice', 'before')).add(KB.html.radio(options.afterLabel, 'positionChoice', 'after'))
        }
        return container.build()
    }
    this.render = function() {
        ...
        var form = KB.dom('div').add(KB.dom('div').attr('id', 'message-container').build()).add(KB.html.label(options.swimlaneLabel, 'form-swimlanes')).add(buildSwimlaneSelect()).add(KB.html.label(options.columnLabel, 'form-columns')).add(buildColumnSelect()).add(buildTasks()).build();
        containerElement.appendChild(form)
    }
});
KB.onClick('.js-template',
function(e) {
...
function(containerElement, options) {
    ...
    function buildViewMode() {
        var toolbarElement = KB.dom('div').attr('class', 'text-editor-toolbar').
        for ('a', [{
            href: '#',
            html: '<i class="fa fa-pencil-square-o fa-fw"></i> ' + options.labelWrite,
            click: function() {
                toggleViewMode()
            }
        }]).build();
    ...
    function buildWriteMode() {
        ...
        for ('a', [{
            href: '#',
            html: '<i class="fa fa-eye fa-fw"></i> ' + options.labelPreview,
            click: function() {
                toggleViewMode()
            }
        },
        ...
        {
            href: '#',
            html: '<i class="fa fa-strikethrough fa-fw"></i>',
            click: function() {
                insertEnclosedTag('~~')
            }
        },
    ...
    function toggleViewMode() {
        KB.dom(previewElement).html(marked(textarea.value, {
            sanitize: !0
        }));
    ...
    function insertText(replacedText) {
        ...
        if (document.queryCommandSupported('insertText')) {
            result = document.execCommand('insertText', !1, replacedText)
        }
        if (!result) {
            try {
...
Kanboard.App.prototype.focus = function() {
    $(document).on('focus', '.auto-select',
    function() {
        $(this).select()
    });
    $(document).on('mouseup', '.auto-select',
    function(e) {
        e.preventDefault()
    })
};
...
Kanboard.BoardColumnView.prototype.listen = function() {
    var self = this;
    $(document).on("click", ".board-toggle-column-view",
    function() {
...
Kanboard.Dropdown.prototype.listen = function() {
    ...
    $(document).on('click', '.active-dropdown-menu',
    function() {
    ...
    $(document).on('click', '.dropdown-submenu-open li',
    function(e) {
        if ($(e.target).is('li')) {
...
Kanboard.Swimlane.prototype.listen = function() {
    var self = this;
    $(document).on('click', ".board-swimlane-toggle",
    function(e) {
        e.preventDefault();
...
Kanboard.Task.prototype.renderColorPicker = function() {
    function renderColorOption(color) {
        return $('<div class="color-picker-option">' + '<div class="color-picker-square color-' + color.id + '"></div>' + '<div class="color-picker-label">' + color.text + '</div>' + '</div>')
    }
